...
| `ChatList.vue` (F7, Droga B) | agregowana lista „Rozmowy" wyklucza rozmowy z etykietami `HIDDEN_LABELS` (`['spam','kurier','marketplace']`); pomija gdy folder/filtr, widok etykiety (`props.label`) LUB konkretny inbox (`activeInbox`) | F7: szum znika z domyślnego widoku (po F6 „rozwiąż" już nie chowa), ale zostaje dostępny w `#etykieta`/folderze/inboxie/szukajce | niska–średnia (computed `conversationList`); licznik zakładki nieskorygowany (kosmetyka) | rozmowa z `spam`/`kurier`/`marketplace` znika z „Rozmowy", widoczna w widoku etykiety i w swoim inboxie (np. allegro@) |
