...
| `helper/agentColor.js` (nowy, F2) + `ConversationCard.vue` + `MessagePreview.vue` | F2: avatar agenta zamiast ikony „osoba" + nazwa w deterministycznym kolorze (hash ID → paleta n-*) + podgląd wiadomości w 2 liniach (prop `multiline`, domyślnie 1 — bez wpływu na inne użycia) | szybki skan „kto prowadzi" + więcej kontekstu wiadomości | średnia (karta + współdzielony MessagePreview) | każdy agent inny kolor, ten sam agent zawsze ten sam; podgląd do 2 linii |
