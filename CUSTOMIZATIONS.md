...
| `ConversationCard.vue` (legacy/domyślny) + `components-next/.../ConversationCardExpanded.vue` | wariant C: neutralne tło, status = lewy pasek + chip (prawy górny róg); active = niebieski pasek + subtelny ciemniejszy panel (alpha-1), hover = jaśniejszy overlay (alpha-2) | F6: widać status, odróżnia nieobsłużone od obsługiwanych; obie karty (lista przełącza wg ustawienia layoutu) | średnia (struktura kart) | Nowy=bez koloru, W toku=amber, Rozwiązane=teal, Oczekujące=ruby, Uśpione=slate; active niebieski, bez ringu |
